...
resDir="/data/projects/2023/LCBiome/nsclc_gender_atlas_tmp/out/007_re_analysis/figures/volcano/" 
c1="male"
c2="female"
cond_col ="sex"
contrast = c(cond_col, c1, c2)
...
chrom_deg_merged <- merge(resIHW_gene_name, chrom[, c("gene_id", "chromosome_name")], by = "gene_id", all.x = TRUE)
chrom_deg_merged_x <- subset(chrom_deg_merged, chromosome_name == "X")
vx <- EnhancedVolcano(chrom_deg_merged_x,
                     lab = chrom_deg_merged_X$gene_name,
                     x = 'log2FoldChange',
                     y = 'pvalue')
vx
ggsave(paste0(resDir,prefix,"_x_chrom_deg_all_volcano_plot.jpg"), plot = vx, width = 8, height = 10)  
chrom_deg_merged <- merge(resIHW_gene_name, chrom[, c("gene_id", "chromosome_name")], by = "gene_id", all.x = TRUE)
chrom_deg_merged_y <- subset(chrom_deg_merged, chromosome_name == "Y")
vy <- EnhancedVolcano(chrom_deg_merged_y,
                      lab = chrom_deg_merged_y$gene_name,
                      x = 'log2FoldChange',
                      y = 'pvalue')
vy
ggsave(paste0(resDir,prefix,"_y_chrom_deg_all_volcano_plot.jpg"), plot = vy, width = 8, height = 10)  


chrom_deg_merged <- merge(resIHW_gene_name, chrom[, c("gene_id", "chromosome_name")], by = "gene_id", all.x = TRUE)
chrom_deg_merged_autosomal <- chrom_deg_merged[!(chrom_deg_merged$chromosome_name %in% c("X", "Y")), ]
vauto <- EnhancedVolcano(chrom_deg_merged_autosomal,
                      lab = chrom_deg_merged_autosomal$gene_name,
                      x = 'log2FoldChange',
                      y = 'pvalue')
vauto
ggsave(paste0(resDir,prefix,"_autosomal_chrom_deg_all_volcano_plot.jpg"), plot = vauto, width = 8, height = 10)  
